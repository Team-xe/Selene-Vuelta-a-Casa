...
public class SueloMultijugador : MonoBehaviour
{
    SueloRecicladoMultijugador suelos;


    void Start()
    {
        //sueloGenerador = GameObject.FindObjectOfType<SuelosGenerador>(); // Guarda el Script SuelosGenerador (buscandolo)
        suelos = GameObject.FindObjectOfType<SueloRecicladoMultijugador>(); // Guarda el Script SuelosGenerador (buscandolo)
    ...
    **/
    ...
    void GenerarEnemigo()
    {
        ...
        while (enemigoGenIndex == obstaculoGenIndex)
        {
            enemigoGenIndex = Random.Range(2, 5);
        }
